...
void solve(){
    ...
    forn(i, n) cin >> arr[i];
    sort(all(arr));
    ld ans=0,ans1=0,ans2=0;
    //printDS(arr);
    if(n==5){
        //set<int> st11({arr[3],arr[4]});
        //set<int> st12({arr[0], arr[1],arr[2]});
        //set<int> st21({arr[2],arr[3], arr[4]});
        //set<int> st22({arr[0], arr[1]});
        //int s11 = 0,s12=0,s21=0,s22=0;
        //for(auto x : st11) s11+=x;
        //for(auto x : st12) s12+=x;
        //for(auto x : st21) s21+=x;
        //for(auto x : st22) s22+=x;
        //ans1 = abs(((ld)s11/(ld)st11.size())-((ld)s12/(ld)st12.size()));
        //ans2 = abs(((ld)s21/(ld)st21.size())-((ld)s22/(ld)st22.size()));
        //deb2(ans1,ans2);
        ans1 = ((arr[3]+arr[4])/(ld)2) - ((arr[0]+arr[2])/(ld)2);
        ans2 = ((arr[2]+arr[4])/(ld)2) - ((arr[0]+arr[1])/(ld)2);
        ans = max(ans1,ans2);
    }
    else{
        ans  = ((ld)(arr[n-1] + arr[n-2]) / (ld)2) - ((ld)(arr[0] + arr[1]) / (ld)2);
    }
    cout << precise(ans, 9)<< endl;
}
