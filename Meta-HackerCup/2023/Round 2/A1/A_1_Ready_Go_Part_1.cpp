...
bool best_white(vector<string> &board,int x, int y)
{
    int side = 0;
    int black = 0;
    int dot = 0;
...
bool dfs(vector<string> &board, vector<vector<bool>> &visited, int x, int y)
{
    visited[x][y] = true;
    int side = 0;
    int black = 0;
    ...
    if(dot)
            return 0;
    if(dot==0 and black==side) return 1;
    bool poss=1;
    forn(i,4){
        int nx = x + dx[i];
        int ny = y + dy[i];
        if (is_valid(nx, ny) && board[nx][ny] == 'W' && !visited[nx][ny])
        {
            poss = (poss&dfs(board, visited, nx, ny));
        }
    }
    return poss;
}

bool capture(vector<string> &board)
{
    vector<vector<bool>> visited(R, vector<bool>(C, 0));

    forn(i,R){
        forn(j,C){
            if (board[i][j] == 'W' && best_white(board,i,j)){
                visited.clear();
