...
//#pragma GCC target("sse,sse2,sse3,ssse3,sse4,popcnt,bm,mmx,avx,avx2,fma")
...
pair<bool,int> _search(vector<array<int, 2>>  &a,int target){
    int l=0, r=a.size()-1;
    while(l<=r){
        int mid = (l+r)/2;
        if(a[mid][0]==target) return mp(true,a[mid][1]);
        if(a[mid][0]>target) r = mid-1;
        if(a[mid][0]<target) l = mid+1;
    }
    return mp(false,-1);
}
...
void solve(){
    int n;
    cin >> n;
    vector<array<int, 2>> a(n), b(n), ways;
    unordered_map<int,array<int,2>> cache;
    forn(i, n) cin >> a[i][0], a[i][1] = i + 1, cache[i+1][0] = a[i][0];
    forn(i, n) cin >> b[i][0], b[i][1] = i + 1, cache[i+1][1] = b[i][0];
    sort(all(a));
    sort(all(b));
    int operation=0;
    forn(i,n){
        if(cache[b[i][1]][0]>b[i][0]){
            neg;
            return;
        }
        if(cache[b[i][1]][0]==b[i][0]) continue;
        // find the target and set it if possible
        auto _data = _search(a, b[i][0]);
        if(_data.first){
            operation++;
            ways.pb({b[i][1], _data.second});
        }
        else{
            neg;
            return;
