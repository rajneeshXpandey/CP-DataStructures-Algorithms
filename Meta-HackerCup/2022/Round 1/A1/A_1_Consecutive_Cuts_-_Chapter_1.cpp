...
void solve(){
    ...
    forn(i, n) cin >> a[i];
    forn(i, n) cin >> b[i];
    vector<array<int,2>> deck(n);
    forn(i,n){
        deck[a[i]-1][0] = i;
        deck[b[i]-1][1] = i;
    }  

    int _move = (n - deck[0][0] + deck[0][1]) % n;
    _move = min(_move,(n + deck[0][0] - deck[0][1]) % n);

    forn(i,n){
        int val = (n - deck[i][0] + deck[i][1]) % n;
        val = min(val, (n + deck[0][0] - deck[0][1]) % n);
        //deb2(deck[i][0] , deck[i][1]);
        //deb2(val,_move);
        if (val != _move)
        {
            no;
