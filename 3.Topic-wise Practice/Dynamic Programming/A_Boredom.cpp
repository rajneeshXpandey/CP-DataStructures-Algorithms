...
int ansMax(int sz,int idx) {
    
    if(idx < sz  and dp[idx]!=-1) return dp[idx];
    if(idx >= sz) return 0;

    int ans1 = 0, ans2 = v[idx] * mpp[v[idx]];
    // dont remove
    ans1 = ansMax(sz,idx + 1);
    // remove
    if(idx<(sz-1) and v[idx] != v[idx+1]-1){
        ans2 += ans1;
    }
    else
        ans2 += ansMax(sz,idx + 2);
...
void solve1(){
    ...
    for (int i = 1; i < n; i++)
    {
        ...
        // take
        if ((v[i] - 1) != v[i - 1])
            dp[i][1] = mp[v[i]] * v[i] + max(dp[i - 1][1], dp[i - 1][0]);
        else
