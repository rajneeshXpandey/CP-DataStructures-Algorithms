...
bool dfs(vector<set<int>> &adj, vector<bool> &visited, vector<int> &parent, int root, int par, int &start, int &end)
{
    visited[root] = 1;
    parent[root] = par;
    ...
    {
        if (!visited[child]){
            if(dfs(adj, visited, parent, child, root, start, end)) 
                // require so that further calls of recursion should be avoided and we should get the desire cycle
                return true;
        }
        else
        {
...
void solve()
{
    ...
    forn(i,n){
        if(!visited[i]){
            if(dfs(adj, visited, parent, i, -1, start, end)){
                possible=1;
                break;
            }
        }
