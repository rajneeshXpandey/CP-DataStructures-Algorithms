...
{  
    ...
    forn(i,n){
        cin>>s>>e>>rew;
        v[i][0]=e; v[i][1]=s; v[i][2]=rew;
    }
    sort(all(v));
    forn(i,n) endtime[i]=v[i][0];
    vector<int> dp(n,0);
    dp[0] = v[0][2];
    rep(i,1,n){
        int idx = lower_bound(all(endtime),v[i][1])-endtime.begin();
        //deb(idx);
        if(idx==0)
            dp[i] = max(dp[i-1],v[i][2]);
        else{
            idx--;
            dp[i] = max(dp[i-1], v[i][2]+dp[idx]);
          }
    }
    //forn(i,n) cout<<dp[i]<<' ';
    cout<<dp[n-1]<<endl;
