...
int findPartition(vector<int> &x, int &desiredSubarraySum, int &n){
    int totalPartition = 1, currentSubarraySum=0;
    for(int i=0;i<n;i++){
        if ((currentSubarraySum + x[i]) > desiredSubarraySum){
            totalPartition++;
...
void solve()
{
    ...
    vector<int> x(n);
    int l=0,r=0;
    forn(i, n) cin >> x[i], l = max(l,x[i]), r += x[i];
    /*
        -> max-sum ranges --> max(all(xi)) to sum(all(xi))
        -> now check for all possible values of max-sum of subarray (each subarray sum <= possible-max-sum) as ans
           and check if there is possibilty for dividing the array into k subarray
        -> is it monotonic?
           yes if partition <= k :- decrease desired subarray sum else if partition > k :- increase desired subarray sum
    */
    int ans = 0;
    while(l<=r){
        int desiredSubarraySum = (l+r)/2;
        int totalPartition = findPartition(x, desiredSubarraySum, n);
        //deb4(l, r, desiredSubarraySum, totalPartition);
        if (totalPartition <= k)
        {
            ans = desiredSubarraySum;
            r = desiredSubarraySum-1;
        }
        else{
            l = desiredSubarraySum+1;
        }
    }
    //assert(l==r);
    cout<<ans<<endl;
