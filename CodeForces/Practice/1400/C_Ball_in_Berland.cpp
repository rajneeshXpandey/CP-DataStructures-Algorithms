...
void solve(){
    ...
    map<int, vector<int>> bPoint;
    forn(i,k){
        pairs[i][0] = a[i];
        pairs[i][1] = b[i];
    }
    sort(all(pairs), [](array<int, 2> &x, array<int, 2> &y){
        if(x[0]==y[0])
            return (x[1] < y[1]);
        return (x[0] < y[0]);
    });
    ...
    forn(i, k)
    {
        int idx = upper_bound(all(a),a[i])-a.begin();
        int bidx = lower_bound(all(bPoint[b[i]]),idx)-bPoint[b[i]].begin();
        int fact = sz(bPoint[b[i]]) - bidx;
        //deb3(idx, bidx, fact);
        ans += (k - idx) - fact;
    }
    cout<<ans<<endl;
