...
void solve()
{
    int n,k,num,max_coin=0;
    cin >> n >>k;
    vector<int> b(n),c(n);
    forn(i, n) cin >> b[i];
    forn(i, n) cin >> c[i],max_coin += c[i];
    int mxx = *max_element(all(b));
    //int mxx = 1000;
    vector<int> steps(mxx+1,inf);
    steps[1] = 0;
    for(int i = 1; i <= mxx; i++)
    {
        for(int j = i; j >= 1; j--)
        {
            num = i + i/j;
            if(num<=mxx)
                steps[num] = min(steps[num], steps[i] + 1);
            else break;    
        }
    }
    //printArr(steps);
    ...
    */
    for(int i=1;i<=n;i++)
    {
        ...
        {
                if(j>=steps[b[i-1]])
                    dp[i][j] = dp[i-1][j-steps[b[i-1]]] + c[i-1];
