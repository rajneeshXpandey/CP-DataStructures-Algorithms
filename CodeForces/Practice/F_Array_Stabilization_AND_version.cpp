...
void solve(){
    ...
    for(auto cyc : cycles){
        ...
        int cont1 = 0;
        int sz = sz(cyc);
        cyc.insert(cyc.end(),cyc.begin(),cyc.end());
        for(int j=0;j<sz(cyc);){
            if(cyc[j]==1){
                int len=0;
                while(j<sz(cyc) and cyc[j]==1){
                    len++;
                    j++;
                }
                cont1 = max(cont1,len);
        ...
        ans = max(ans,cont1); 
        // oprerations occurs in parallel so maxx will be the max operation and 
        // remaining will automaticalyy completed by them. 
    }
    cout<<ans<<endl;
}
