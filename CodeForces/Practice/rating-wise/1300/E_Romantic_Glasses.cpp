...
void solve(){
    int n;
    cin >> n;
    vector<int> arr(n), a(n+1,0),b(n+1,0);
    loop(i,1,n){
        cin>>arr[i-1];
        if(i%2==0){
            a[i] = (i == 0) ? 0 : (a[i-1]);
            b[i] = (i == 0)?arr[i-1]:(b[i-1]+arr[i-1]);
        }
        else{
            a[i] = (a[i-1]+arr[i-1]);
            b[i] = b[i-1];
    ...
    //printDS(a);
    //printDS(b);
    map<int,int> pair_cnt;
    loop(i,0,n){
        pair_cnt[a[i] - b[i]]++;
        if(pair_cnt[a[i]-b[i]]>1){
            YES;
            return;
        }
    }
...
/**
    ...
    a -> a1 a2 a3 a4 a5 :- prefix sum of even pos (ai = arr[1]...arr[i])

    b -> b1 b2 b3 b4 b5 :- prefix sum of odd pos (bi = arr[0]...arr[i-1])

    we want a[l]-a[r] = b[l]-b[r] 
        => a[l] - b[l] == a[r] - b[r]
 */
