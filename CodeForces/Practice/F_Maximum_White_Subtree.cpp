...
int dfs(int par, int node,
        vector<vector<int>>& tree, vector<int>& good,
        vector<int>& subtreemax) {
    subtreemax[node] = (good[node]==1)?(1):(-1);
    for (auto child : tree[node]) {
        if (child != par) {
            int childsum = dfs(node, child, tree, good, subtreemax);
            subtreemax[node] += (childsum>0)?childsum:0;
        }
    }
    return subtreemax[node];
}

void maximumScore(int par, int node,
                vector<vector<int>>& tree, vector<int>& dp,
                vector<int>& subtreemax) {
    if (node != 0)
        dp[node] = max((dp[par]-max(subtreemax[node],0ll)),0ll)+subtreemax[node];
    for (auto child : tree[node]) {
        if (child != par) {
            maximumScore(node, child, tree, dp, subtreemax);
        }
    }
}
...
void solve(){
    ...
    vector<int> dp(n, 0), subtreemax(n, 0);
    dfs(-1, 0, tree, good, subtreemax);
    dp[0] = subtreemax[0];
    maximumScore(-1, 0, tree, dp, subtreemax);
    forn(i,n) cout<<dp[i]<<' ';
}
