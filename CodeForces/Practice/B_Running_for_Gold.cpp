...
void solve(){
    ...
    int u = 0;
    // if u is superior to i : do nothing else u = i 
    // only one can be superior to all others if two are superior then they should not mutually superior to each other
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < 5; j++) {
            cin >> arr[i][j];
        }
    ...
    // check if u is superior to all others
    for (int i = 0; i < n; i++) {
        if (u == i) {
            continue;
        }
