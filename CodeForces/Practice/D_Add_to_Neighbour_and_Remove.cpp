...
void solve(){
    ...
    auto process = [&](vector<int> &arr,int &ai,int &size,int &op){
        int r=0,tsum=0,cnt=0,l=0;
        while(r<n){
            if(tsum<ai) tsum+=arr[r];
            if(tsum>ai) return false;
            else if(tsum==ai){
                tsum=0;
                op+=r-l;
                l=r+1;
