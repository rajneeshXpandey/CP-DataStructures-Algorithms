...
int dfs(int par, int node, int dist,int &rootnode, vector<vector<int>> &tree, vector<int> &arr, vector<int> &subtreesum)
{
    subtreesum[node] += arr[node-1];
    rootnode += (dist*arr[node-1]);
    for(auto child : tree[node]){
        if(child != par){
            int childsum = dfs(node,child,dist+1,rootnode,tree,arr,subtreesum);
            subtreesum[node] += childsum;
        }
    }
    return subtreesum[node];
}

int maximumCost(int &ans, int &totsum, int par, int node, vector<vector<int>> &tree, vector<int> &dp, vector<int> &subtreesum)
{
    if(node != 1)
        dp[node] = totsum - subtreesum[node] + dp[par] - subtreesum[node];
    for (auto child : tree[node])
    {
        if (child != par){
            maximumCost(ans, totsum,  node, child, tree, dp, subtreesum);
        }
    }
    ans = max(ans, dp[node]);
}
...
void solve(){
    ...
    vector<int> dp(n+1,0), subtreesum(n+1,0);
    int rootnode=0;
    dfs(0, 1, 0, rootnode, tree, arr, subtreesum);
    int ans = 0;
    dp[1] = rootnode;
    int totsum = subtreesum[1];
    maximumCost(ans, totsum, 0, 1, tree, dp, subtreesum);
    cout<<ans<<endl;
}

/**

=> 0*[9] + (1*4 + 2*1) + (1*10 + 2*1+ 2*6 + 2*5) + (1*7)

=> (0*[4] + 1*9 + 1*1 + 2*10 + 3*1+ 3*6 + 3*5 + 2*7)


dp[node] = dp[par] + subtreesum[par] - 2*subtreesum[node]
