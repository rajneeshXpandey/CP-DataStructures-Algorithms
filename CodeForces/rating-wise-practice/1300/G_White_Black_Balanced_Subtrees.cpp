...
array<int,2> dfs(vector<vector<int>> &adj, int node, int parent, int &balanced_subtrees, string &bw){
    array<int, 2> blackAndWhite = {0,0};
    if(bw[node-1]=='B') 
        blackAndWhite[0]++; 
    else 
        blackAndWhite[1]++;
    for(auto &child : adj[node]){
        ...
        auto clrs = dfs(adj, child, node, balanced_subtrees, bw);
        blackAndWhite[0] += clrs[0];
        blackAndWhite[1] += clrs[1];
    }
...
void solve(){
    ...
    dfs(adj,1ll,-1ll,balanced_subtrees,bw);
