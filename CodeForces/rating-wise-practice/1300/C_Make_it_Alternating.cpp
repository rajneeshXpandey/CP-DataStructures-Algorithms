...
void solve(){
    ...
    cin>>bs;
    int n=bs.size();
    int min_operation=0, ways=1ll, curr_same=0;
    loop(i,1,n-1){
        if(bs[i]==bs[i-1]){
            curr_same++;
        }
            ...
            if(curr_same){
                min_operation += (curr_same);
                ways = (ways * (curr_same + 1)) % mod;
                curr_same = 0;
            }
        }
    }
    ...
    ways = (ways*factorial[min_operation])%mod;
    cout<<min_operation<<' '<<ways<<endl;
}
