...
void solve(){
    ...
    cin >> n;
    // consolidate the column-wise movement and armor
    forn(i, n){
        cin>>k;
        maxkj = 0;
        forn(j,k){
        ...
        arr.push_back({maxkj,k-1});
    }
    sort(all(arr));

    // check if power is sufficient
    auto check_if_power_suff = [&](int pwr){
        int column_effect = 0;
        forn(i, n){
            if ((arr[i][0] - column_effect) >= pwr)
                return false;
            column_effect += arr[i][1];
            pwr++;
        }
        return true;
    };
    ...
    // find best answer 
    int ans=0, l=1,r=1e9+5, mid;
    while (l <= r){
        mid=(l+r)/2;
        if(check_if_power_suff(mid)){
            ans = mid;
            r=mid-1;
        }
        else l=mid+1;
