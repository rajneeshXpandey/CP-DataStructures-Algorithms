...
int find_compressed_index(vector<pair<array<int, 2>, int>> &arr, int idx){
    int l = 0, r = arr.size() - 1;
    while(l<=r){
        int mid = (l+r)/2;
        if(idx>=arr[mid].first[0] and idx<=arr[mid].first[1])
            return mid;
        else if(idx>arr[mid].first[1]){
            l = mid+1;
        }
        else if(idx<arr[mid].first[0]){
            r = mid-1;
        }
    }
    return -1;
...
void solve(){
    ...
    vector<int> arr(n);
    forn(i, n) cin >> arr[i];
    vector<pair<array<int,2>,int>> compressed_array;
    forn(i,n){
        if(i==0) {
            compressed_array.pb({{1,1},arr[i]});
            continue;
        }
        if(arr[i]==arr[i-1]){
            compressed_array.back().first[1] = i+1;
        }
        else{
            compressed_array.pb({{i+1, i+1}, arr[i]});
        }
    }
    int q,l,r,mapped_l,mapped_r;
    cin>>q;
    forn(i, q){
        cin>>l>>r;
        mapped_l = find_compressed_index(compressed_array, l);
        mapped_r = find_compressed_index(compressed_array, r);
        if(mapped_l==-1 || mapped_r==-1 || mapped_l==mapped_r) {cout<<"-1 -1"<<endl; continue;}
        if (compressed_array[mapped_l].second == compressed_array[mapped_r].second){
            cout<<l<<' '<<compressed_array[mapped_r-1].first[1]<<endl;
        }
        else{
            cout<<l<<' '<<r<<endl;
        }
