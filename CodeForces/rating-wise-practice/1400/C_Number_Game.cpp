...
bool buildResult(map<int,bool> &cache,int x){

    if(cache.find(x)!=cache.end()) return cache[x];

    if(x==1) return cache[1] = 0;
    else if(x==2) return cache[2] = 1;
    else if(x&1) return cache[x] = 1;

    vector<int> oddFactors_of_x;

    for(int i=1;i*i<=x;i++){
        if(x%i==0){
            if(i>1 and i&1)
                oddFactors_of_x.pb(i);
            if (i != (x / i) and (x / i)>1 and (x / i) & 1)
                oddFactors_of_x.pb(x / i);
        }
    }
    sort(rall(oddFactors_of_x));
    //printArr(oddFactors_of_x);
    for (auto facs : oddFactors_of_x){
        if(!buildResult(cache,x/facs)){
            return cache[x] = 1;
        }
    }
    return cache[x] = !(buildResult(cache, x-1));
}
void solve(){
    int n;
    cin >> n;
