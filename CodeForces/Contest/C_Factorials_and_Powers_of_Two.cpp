...
// *********************** Code Begins ************************ //
vector<int> fact(16,1);
vector<int> pow2(41,1);
set<pii> sums;
void solve()
{
    int n;
    cin >> n;
    int maxfact = lower_bound(fact.begin(), fact.end(), n) - fact.begin();
    if(maxfact>=16) maxfact=15;
    //for(auto f :fact) cout<<f<<' ';
    //cout<<endl;
    //deb(maxfact);
    if(fact[maxfact]>n){
        maxfact--;
