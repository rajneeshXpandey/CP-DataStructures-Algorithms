...
// ********************************* start ********************************** //
void dfs(int node,int par,vector<vector<pii>> &adj,int idx,int &read,int itr)
{
    read = max(read, itr);
    for(auto child : adj[node]){
        if(child.ff==par) continue;
        dfs(child.ff,node,adj,child.ss,read,itr+(child.ss<idx));
    }
}
void solve(){
    int n;
