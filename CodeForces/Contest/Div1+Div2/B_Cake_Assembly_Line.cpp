...
void solve(){
    ...
    auto check = [&](int move){
        forn(i, n){
            if(a[i]-w+move > b[i]-h) return false;
            if(a[i]+w+move < b[i]+h) return false;
        }
        return true;
    };
    int mnmove = inf, mxmove = -inf, _mnmove = inf, _mxmove = -inf;
    forn(i, n){
        amin(mnmove, b[i]-h-a[i]+w);
        amax(mxmove, b[i]-h-a[i]+w);
        amin(_mnmove, b[i]+h-a[i]-w);
        amax(_mxmove, b[i]+h-a[i]-w);
    }
    int case1 = check(mnmove), case2 = check(mxmove), case3 = check(_mnmove), case4 = check(_mxmove);
