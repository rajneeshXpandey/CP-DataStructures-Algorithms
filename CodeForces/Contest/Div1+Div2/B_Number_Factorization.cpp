...
void solve(){
    ...
    cin >> n;
    map<int, int> mp;
    prime_factorization(n,mp);
    multiset<int> minpow,temp;
    for(auto p:mp)
        minpow.insert(p.ss);
    int ans=0;
    int tans=1;
    while(size(minpow)>0){
        int po = *minpow.begin();
        minpow.erase(*minpow.begin());
        tans=1;
        for(auto &p : mp){
            //deb3(p.ff,p.ss,po);
            //deb2(p.ff,p.ss);
            //deb(pownums.count(p.ss));
            if(p.ss>0){
                tans = tans*(p.ff);
                p.ss-=po;
            }
        ...
        ans += tans*po;
        for(auto itr=minpow.begin();itr!=minpow.end();itr++){
            int pow = (*itr-po);
            if(pow>0)
                temp.insert(pow);
        }
        minpow.clear();
        minpow.insert(all(temp));
        temp.clear();
    }
    cout<<ans<<endl;
}
