...
void solve(){
    ...
    loop(i,1,n-1){
        if(arr[i] == arr[i-1]){
            ...
            mpa[arr[i]]=max(mpa[arr[i]],j-i+2);
            i=j;
        }
        else{
            if(i==1) mpa[arr[0]]=max(mpa[arr[0]],1LL);
            mpa[arr[i]]=max(mpa[arr[i]],1LL);
        }
    }
    loop(i,1,n-1){
        if(brr[i] == brr[i-1]){
            int j = i-1;
            while(j+1<n and brr[j] == brr[j+1]) j++;
            mpb[brr[i]]=max(mpb[brr[i]],j-i+2);
            i=j;
        }
        else{
            if(i==1) mpb[brr[0]]=max(mpb[brr[0]],1LL);
            mpb[brr[i]]=max(mpb[brr[i]],1LL);
        }
    }
    int ans=1;
    for(auto x:mpa){
        ans=max(ans,mpa[x.first]+mpb[x.first]);
    }
    for(auto x:mpb){
        ans=max(ans,mpa[x.first]+mpb[x.first]);
    }
