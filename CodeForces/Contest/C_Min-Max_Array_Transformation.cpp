...
void solve(){
    int n;
    cin >> n;
    vector<int> a(n), b(n), min(n), max(n),v;
    forn(i, n) cin >> a[i];
    forn(i, n) cin >> b[i];
//    int right=n-1;
//
//    for(int i=n-1;i>=0;i--){
//        int mxidx = right;
//        //int mnidx = lower_bound(b.begin(),b.begin()+right,a[i])-b.begin();
//        int mnidx = lower_bound(b.begin(),b.begin()+right+1,a[i])-b.begin();
//        //deb2(mnidx,mxidx);
//        min[i] = (b[mnidx]-a[i]); 
//        max[i] = (b[mxidx]-a[i]);
//        if(min[i]==max[i]) right--;
//    }
//    
    loop(i,1,n-1)
    {
