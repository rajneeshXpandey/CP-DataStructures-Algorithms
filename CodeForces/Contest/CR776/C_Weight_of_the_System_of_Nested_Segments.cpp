...
void solve()
{
    ...
    cin >> n >> m;
    vector<pii> a(m), ans;
    forn(i, m) cin >> a[i].ff >> a[i].ss;
    unordered_map<int, int> idx,wt;
    forn(i, m) idx[a[i].ff] = i + 1, wt[a[i].ff] = a[i].ss;
    sort(all(a));
    // deb(h);
    // forn(i,m) {
    ...
    // }
    // forn(i,h) {
    //     cout << left[i].ff << ' ' << left[i].ss << endl;
    // }
    //  rep(i,h,m) {
    //     cout << [i].ff << ' ' << [i].ss << endl;
    // }
    vector<pii> left(m-n),right(m-n),aall(m);
    forn(i,m) aall[i].ff = a[i].ss, aall[i].ss = a[i].ff;
    sort(all(aall));
    vector<int> pts(2*n); 
    forn(i,2*n) pts[i] = aall[i].ss;
    sort(all(pts));
    int l = 0,r=(n*2)-1;
    int twt=0;
    while (l < n and r>=n)
    {
        ans.pb({idx[pts[l]], idx[pts[r]]});
        twt += wt[pts[l]]+wt[pts[r]];
        l++,r--;
    }
    cout << twt << endl;
    forn(i,n) cout << ans[i].ff << ' ' << ans[i].ss << endl;
    cout << endl;
}
