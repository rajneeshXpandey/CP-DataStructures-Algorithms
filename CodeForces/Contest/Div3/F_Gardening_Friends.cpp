...
void diameter(vector<vector<int>> &adj, vector<bool> &visited, vector<int> &disfrom, int root, int distance)
{
    visited[root] = 1;
    disfrom[root] = distance;
    for (auto child : adj[root])
    {
        if (!visited[child])
        {
            diameter(adj, visited, disfrom,child,distance+1);
        }
    }
}
...
void solve(){
    ...
    vector<int> disfromRoot(n, 0);
    vector<int> disfromNode(n, 0);
    vector<int> disfromNode1(n, 0);
    int maxDis = -1,node=0;
    diameter(adj, visited, disfromRoot, 0, 0);
    forn(i, n){
        if(disfromRoot[i] > maxDis){
            maxDis = disfromRoot[i];
            node = i;
        }
    }
    maxDis = -1;
    visited.clear();
    visited.resize(n, 0);
    diameter(adj, visited, disfromNode, node, 0);
    int node1 = 0;
    forn(i, n){
        if(disfromRoot[i] > maxDis){
            maxDis = disfromRoot[i];
    ...
    visited.clear();
    visited.resize(n, 0);
    diameter(adj, visited, disfromNode1, node1, 0);
    int ans=-inf;
    forn(i,n){
        int tans = max(k*disfromNode[i],k*disfromNode1[i])-(c*disfromRoot[i]);
        ans = max(ans,tans);
    }
    cout<<ans<<endl;
}
