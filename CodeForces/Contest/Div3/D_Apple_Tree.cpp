...
int dfs(vector<vector<int>> &adj, vector<int> &leafnodes, int par, int root){
    int lfnodes = 0;
    bool childh = 0;
    for(auto child : adj[root]){
        if(child == par) continue;
        childh=1;
        lfnodes += dfs(adj, leafnodes, root, child);
    }
    if(!childh)
        lfnodes++;
    return leafnodes[root] = lfnodes;
}
void solve(){
    int n,u,v,q,x,y;
    cin >> n;
    ...
    vector<int> leafnodes(n+1,0);
    dfs(adj,leafnodes,-1,1);
    //printDS(leafnodes);
    cin>>q;
    int ans=0;
    while(q--){
        cin >> x >> y;
        ans = (leafnodes[x]*leafnodes[y]);
        cout << ans << endl;
    }
}
