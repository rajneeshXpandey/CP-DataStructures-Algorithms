...
void solve(){
    ...
    cin >> w >> m;
    /*
       we are interested in 0 - 8 bcz % 9
        if sum of dig % 9 == 0 means num is div by 9
        else remainder is sum of dig%9;  
    */
    vector<int> f(n + 1); // prefix sum
    for (int i = 0; i < n; i++)
    {
        f[i + 1] = f[i] + s[i] - '0';
    }
    ...
    vector<int> a[9];
    for (int i = 0; i + w <= n; i++) // storing w length long num%9
    {
        int x = (f[i + w] - f[i]) % 9;
        a[x].push_back(i);
    }
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        int x = (f[r] - f[l - 1]) % 9; // x with mod 9
        array<int, 2> ans{n, n};
        for (int u = 0; u < 9; u++)
        {
            if (a[u].size() > 1 && (u * x + u) % 9 == k) // checking for same remainder value
            {
                ans = min(ans, array{a[u][0], a[u][1]});
            }
            for (int v = 0; v < 9; v++) // for other remaiders
            {
                if (u != v && !a[u].empty() && !a[v].empty() && (u * x + v) % 9 == k)
                {
                    ans = min(ans, array{a[u][0], a[v][0]});
                }
            }
        }
