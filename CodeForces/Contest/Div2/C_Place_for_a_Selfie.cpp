...
void solve()
{
    ...
    forn(i, n) cin >> k[i];
    forn(i, m) cin >> abc[i][0] >> abc[i][1] >> abc[i][2];
    int a, b, c;
    sort(all(k));
    forn(i, m)
    {
        // 4ac > (b-k)*(b-k)
        // k^2 - 2kb + b^2 - 4ac < 0
        // if(at k==b) y >= 0 ans is no || a*c< 0 then also no;
        // else k :- [-1-4*sqrt(ac)/2  , -1+4*sqrt(ac)/2] // total 4*sqrt(ac) values
        bool ok = true;
        a = abc[i][0], b = abc[i][1], c = abc[i][2];
