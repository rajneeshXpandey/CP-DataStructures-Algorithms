...
// ********************************* Here we go!! ********************************** //
pii givelr(vector<int> &arr, int l, int r,int al,int ar,map<int, int> &m){
    if(l>=r) return {-1,-1};
    int li = m[al], ri = m[ar];
    //if(li>ri) return {-1, -1};
    //deb4(l, r, li,ri);
    if(li>l and li<r and ri<r and ri>l){
        return {l, r};
    }
    else if(ri==r){
        return givelr(arr, l, r-1, al,ar-1, m);
    }
    else if(li==l){
        return givelr(arr, l+1, r, al+1,ar, m);
    }
    else if(li==r){
        return givelr(arr, l, r-1, al+1,ar, m);
    }
    else if(ri==l){
        return givelr(arr, l+1, r, al,ar-1, m);
    }
    return {-1, -1};
}
void solve(){
    ...
    auto ans  = givelr(arr, 0, n-1,1,n,m);
    if(ans.ff==-1){
        cout<<-1<<endl;
    }
    else{
