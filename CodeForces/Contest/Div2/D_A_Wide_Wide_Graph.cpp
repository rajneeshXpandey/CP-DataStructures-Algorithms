...
void solve(){
    ...
    int x = bfs(0);
    int y = bfs(x);
    auto dx = dist; // distance from x to all nodes
    bfs(y);
    auto dy = dist; // distance from y to all nodes

    // x and y are the endpoints of the diameter

    /*
        input graph test case 1:
    ...
    */
    ...
    // node make diff comp if its max dist node  distance is greater than other k
    vector<int> ans(n + 1);
    for (int i = 0; i < n; i++)
    {
        ans[max(dx[i], dy[i])+1] += 1;  // components added by this node
    }
    ans[0] += 1; // base case
    ans[dx[y] + 1] -= 1; // for repeat count
    loop(i,1,n){
        ans[i] += ans[i - 1];
    }
    loop(i,1,n){
