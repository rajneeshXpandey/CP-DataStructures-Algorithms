...
void solve(){
    ...
    int ans = inf;
    /*
      fix the k length of the leg
      then we can find the number of steps required to reach (x,y) = ceil(x/k) + ceil(y/k) + k-1

      x/k by len == k and remaining x%k by len == x%k


      this value will give give result in k:- {1,sqrt(max(x,y))}
    */
    for(int k=1;k<=1e5;k++){
        int steps = cdiv(x,k) + cdiv(y,k) + k-1;
        amin(ans,steps);
    }
    cout<<ans<<endl;
}
