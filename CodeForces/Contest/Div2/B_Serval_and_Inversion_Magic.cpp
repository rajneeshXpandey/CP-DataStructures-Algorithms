...
bool makePalindrome(string a, string b, int n){
    int l = 0, r = 0;
    reverse(all(b));
    bool inv = false;
    while(l<n and r<n){
        if(inv and a[l]!=b[r]) return false;
        if(a[l] != b[r]){
            inv = true;
            while(l<n and r<n and a[l] != b[r]){ // inversing the string
                l++;
                r++;
            }
        }
...
void solve(){
    ...
    int l = 0, r = n/2;
    if(n&1) r++;
    string a = s.substr(l, n/2);
    string b = s.substr(r, n/2);
    //deb2(a, b);
    bool ans = (makePalindrome(a, b, n/2) || makePalindrome(b, a, n/2));
    ans ? yes : no;
}
/*
    1234|5678
