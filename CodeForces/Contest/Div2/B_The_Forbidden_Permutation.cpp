...
void solve(){
    int n,m,d;
    cin >> n >> m>>d;   
    vector<int> per(n),arr(m);
    forn(i, n) cin >> per[i];
    forn(i, m) cin >> arr[i];
    vector<int> pos(n+1, -1),posofarr(m, -1);
    loop(i,0,n-1) pos[per[i]] = i+1;
    loop(i,0,m-1) posofarr[i] = pos[arr[i]];
    vector<int> diff(m-1, 0);
    loop(i,0,m-2){
        diff[i] = posofarr[i+1] - posofarr[i];
    }
    // pos(ai+1) - pos(ai) > d should be

    //printDS(posofarr);
    //printDS(diff);
    int ans=inf;
    forn(i, m-1){
        if(diff[i]<0 || diff[i]>d){
            amin(ans,0LL);
        }
        else{
            int leftspace = posofarr[i] - 1;
            int rightspace = n - posofarr[i+1];
            if(leftspace+rightspace+diff[i]>d){
                amin(ans,(d-diff[i]+1LL));
            }
            amin(ans, diff[i]);
        }
    }
