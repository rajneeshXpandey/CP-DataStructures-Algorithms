...
void solve(){
    ...
    forn(i,n){
        if (a[i] == b[i] or (b[i] <= b[(i + 1) % n] + 1)) // assume a[i+1]=b[i+1] formed
            continue;
        no;
        return;
    }
...
/*
a[] = 3 4 1 2
b[] = 6 4 2 5 -> if ai can become bi if bi has reached b(i+1) + 1

lets a[i+1] formed (i.e equal to b[i+1]) then a[i] can form only 
    if b[i](target) that is <=b[i+1]+1 else we cant make it 
*/
signed main(){
    FastIO;
