...
void solve(){
    ...
    cin>>p;
    int n = str.size();
    vector<pair<char,int>> mst;
    int iscore = 0;
    for(int i=0;i<n;i++){
        mst.push_back({str[i],i});
        iscore += str[i]-'a'+1;
    }
    sort(rall(mst));
    // for(auto p:mst){
    ...
    // }
    int it=0;
    while(iscore>p and it<n){
        iscore -= (mst[it].ff)-'a'+1;
        it++;
    }
    // deb(it);
    vector<pair<int,char>> makestr;
    for(int i=it;i<n;i++){
        makestr.push_back({mst[i].ss,mst[i].ff});
    }
    sort(all(makestr));
    string ans = "";
    for(auto val : makestr){
        ans += val.ss;
    }
    cout<<ans<<endl;
}
