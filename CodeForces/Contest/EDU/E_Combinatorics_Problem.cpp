...
inline ll power(ll a, ll n, ll mod){a %= mod; ll res = 1; while(n){if (n & 1) res = (res * a) % mod; a = (a * a) % mod;n >>= 1LL;} return res;} // power in mod
...
void pre() // call this func to precalculate
{
    fac[0] = 1;
    for (int i = 1; i < N; i++)
        fac[i] = (fac[i - 1] * i) % mod; // (a*b)%mod = ((a%mod)*(b%mod))%mod
}
...
void solve(){
    int n, a1, x, y, m, k;
    cin >> n >> a1 >> x >> y >> m >> k;
    ll a = a1 , bk = (nCr(1,k)*a1)%mod , 
        bk_1 = (nCr(1,k-1)*a1)%mod,
        bk_2 = (nCr(1,k-2)*a1)%mod,
        bk_3 = (nCr(1,k-3)*a1)%mod,
        bk_4 = (nCr(1,k-4)*a1)%mod,
        bk_5 = (nCr(1,k-5)*a1)%mod;

    ll ans = bk;
    for(int i=2;i<=n;i++){
        // ai = (a[i-1]*x+y)%m
        a = ((a*x) + y)%m;
        bk = (bk + bk_1 + (nCr(1,k)*a))%mod;
        ans = ans^(bk*i);
        bk_1 = (bk_1 + bk_2 + (nCr(1,k-1)*a))%mod;
        bk_2 = (bk_2 + bk_3 + (nCr(1,k-2)*a))%mod;
        bk_3 = (bk_3 + bk_4 + (nCr(1,k-3)*a))%mod;
        bk_4 = (bk_4 + bk_5 + (nCr(1,k-4)*a))%mod;
        bk_5 = (bk_5 + (nCr(1,k-5)*a))%mod;
    }
    cout<<ans<<endl;
}
