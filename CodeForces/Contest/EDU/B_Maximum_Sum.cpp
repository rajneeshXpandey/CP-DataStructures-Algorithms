...
int maxsum(int l,int r,vector<int> &arr,int k,vector<int> &sum){
    if(l+1>r) return 0;
    if(k<=0) return (sum[r+1]-sum[l]);
    //if(arr[l]+arr[l+1]<arr[r]){
    //    return maxsum(l+2,r,arr,k-1,sum);
    //}
    //else if(arr[l]+arr[l+1]>arr[r]){
    //    return maxsum(l,r-1,arr,k-1,sum);
    //}
    return max(maxsum(l,r-1, arr,k-1,sum),maxsum(l+2,r,arr,k-1,sum));
}
void solve(){
    int n,k;
    cin >> n >> k;
    ...
    //int ans = maxsum(0,n-1,arr,k,sum);
    int ans=0;
    /*
         x+y=k  
         .......2x....|        |.....y.....
