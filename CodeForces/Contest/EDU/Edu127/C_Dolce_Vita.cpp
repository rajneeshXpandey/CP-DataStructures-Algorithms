...
// ********************************* Code Begins ********************************** //
bool poss(int day, vector<int> &prex,int n,int x){
    if((prex[0]+day*1)>x) return 0;
    return 1;
}
void solve(){
    int n,x;
    ...
    rloop(i,n-1,0){
        // can take i+1 items
        if(x<(prex[i])) continue;
        int day=0;
        int poss = ceil(double(x - prex[i] + 1 ) / double(i + 1));
        ans += (i+1)*(poss);
        forn(j,i+1) prex[j] += (j+1)*poss;
        //day += poss;
        //deb3(x,poss,prex[i]);
    }
    cout<<ans<<endl;
