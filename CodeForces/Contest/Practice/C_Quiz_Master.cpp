...
void contribution(unordered_map<int, set<int>> &contrib,vector<int> &a, int &m)
{  
    for(auto x : a){
        for (int f = 1; f * f <= x; f++){
            if (x % f == 0)
...
void solve(){
    ...
    forn(i, n) cin >> a[i];
    sort(all(a));
    unordered_map<int, set<int>> contrib;
    contribution(contrib,a,m);
    map<int,int> mp;
    int l=0,r=0,ans=inf;
    while(r<n){
        if(mp.size()<m){
            for(auto x : contrib[a[r]]){
                mp[x]++;
            }
        }
        if (mp.size() >= m)
