...
//Data structures
#define max_heap priority_queue<pair<int,int>> //maxpq
...
void solve(){
    ...
    map<int,int> cache;
    int sum=0, curr_max;
    forn(i, n) cin >> arr[i], cache[arr[i]]++;
    for(auto p: cache){
        hp.push(p.first);
    }
    vector<int> suffxSum(n+1, 0);
    rloop(i,n-1,0){
        suffxSum[i] = suffxSum[i + 1] + arr[i];
    }
    vector<int> prefxMax(n + 1, 0);
    prefxMax[0] = 0;
    forn(i,n){
        prefxMax[i+1] = max(prefxMax[i], arr[i]);
    }
    loop(k, 1, n)
        cout<<suffxSum[n-k+1]+prefxMax[n-k+1] << " ";
    cout<<endl;
}
