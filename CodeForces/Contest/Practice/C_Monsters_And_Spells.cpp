...
void solve()
{
    ...
    forn(i, n) cin >> h[i];
    /*
      ______k1_______k2________k3______________kn
        -----|     --|                  -------|        
                  -----------------|
    
      -> calc intervals 
      -> merging all the intersecting intervals 
      -> add to ans
     */

     vector<pii> intrvl;
     forn(i,n)
         intrvl.pb({k[i]-h[i]+1,k[i]});

     sort(all(intrvl));

     int st=intrvl[0].ff,end=intrvl[0].ss;
     int ans=0;

     rep(i,1,n){
         if(end>=intrvl[i].ff){
             end = max(end,intrvl[i].ss);
         }
         else{
             ans += (end - st + 1) * (end - st + 2)/2;
             st = intrvl[i].ff, end = intrvl[i].ss;
         }
     }
     ans += (end - st + 1) * (end - st + 2) / 2;
     cout << ans << endl;
