...
void bfs_level(vector<vector<int>> &adj, int root, int &n,vector<bool> &visited, vector<int> &distance,int &ans){
    queue<int> q;
    q.push(root);
    distance[root]=0;
    while (!q.empty()){
        ...
        {
                if (distance[curr]+1LL < distance[nb] and distance[curr]+1LL < ans){   
                    distance[nb] = distance[curr] + 1LL;
                    q.push(nb);
                }
        }
    }
...
void solve(){
    ...
    bfs_level(adj, c0, n,visited, distance,ans);
    for(auto node : c){
        amin(ans,distance[node]);
        bfs_level(adj, node, n,visited,distance,ans);
        cout<<ans<<' ';
    }
    cout<<endl;
}
