...
void solve()
{
    int n;
    cin >> n;
    vector<int> v(n),total(n);
    forn(i, n) cin >> v[i];
    sort(all(v));
    total[0] = v[0];
    rep(i,1,n) total[i]=total[i-1]+v[i];
    bool flag =0;
    //while(l<=r){  
    //    int mid = (l+r)/2;
    //    /*
    ...
    //        0.......mid | mid+1....n-1
    //    */ 
    //    if(total[mid]<(total[n-1]-total[mid]) and (mid+1)>(n-mid-1)){
    //        flag=1;
    //        break;
    //    }
    //    else if(total[mid]>=(total[n-1]-total[mid])){
    //        r=mid-1;
    //    }
    //    else{
    //        l=mid+1;
    ...
    int l=1,r=n-1;
    while(l<r){
        if (total[l] < (total[n - 1] - total[r-1]) and (l + 1) > (n - r))
        {
            flag = 1;
            break;
        }
