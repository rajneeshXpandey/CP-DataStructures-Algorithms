...
void dfs(int root, int par, vector<int> adj[], vector<int> &cnt, vector<int> &wh, vector<int> &bl, vector<char> &col)
{
    cnt[root] = 1;
    if(col[root]=='W')
        wh[root] = 1;
...
void solve(){
    int n,v;
    cin >> n;
    vector<int> cnt(n, 0), par(n-1), wh(n, 0), bl(n, 0);
    vector<char> col(n);
    forn(i,n-1) cin>>par[i],par[i]--;
    string cll;
    cin>>cll;
    forn(i,n){
        col[i]=cll[i];
