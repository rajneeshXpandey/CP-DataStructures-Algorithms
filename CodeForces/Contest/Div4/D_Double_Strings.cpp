...
void solve(){
    ...
    vector<string> a(n);
    forn(i,n) cin >> a[i];
    unordered_map<int,multiset<string>> mp;
    forn(i,n){
        mp[sz(a[i])].insert(a[i]);
    }
    string ans="";
    forn(i,n){
            bool flag=0;
        for(int lsz=1;lsz<sz(a[i]);lsz++){
            int rsz = sz(a[i])-lsz;
            string lstr = a[i].substr(0,lsz);
            string rstr = a[i].substr(lsz,rsz);
            if(mp.find(lsz)!=mp.end()){
                if(mp[lsz].find(lstr)!=mp[lsz].end()){
                    if(mp.find(rsz)!=mp.end()){
                        if(mp[rsz].find(rstr)!=mp[rsz].end()){
                            ans+="1";
                            flag =1;
                            break;
                        }
