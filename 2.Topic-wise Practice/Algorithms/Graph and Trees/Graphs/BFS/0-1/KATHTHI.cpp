...
void _0_1_bfs(vector<vector<int>> &adj, vector<vector<int>> &mindis, int n, int m)
{
    deque<pii> q;
    q.push_back({0, 0});
    mindis[0][0] = 0;
    while (!q.empty())
    {
        ...
        forn(i, 4)
        {
            ...
            if (isValid(x, y, n, m))
            {
                if (adj[x][y] == adj[u.ff][u.ss] and mindis[x][y] > mindis[u.ff][u.ss])
                {
                    q.push_front({x, y});
                    mindis[x][y] = mindis[u.ff][u.ss];
                }
                else if (adj[x][y] != adj[u.ff][u.ss] and mindis[x][y] > mindis[u.ff][u.ss] + 1)
                {
                    q.push_back({x, y});
...
void solve()
{
    ...
    vector<vector<int>> mindis(n, vector<int>(m, inf));
    string s;
    forn(i, n)
    {
        cin >> s;
        forn(j, m) adj[i][j] = s[j] - 'a';
    }
    _0_1_bfs(adj, mindis, n, m);
    cout << mindis[n - 1][m - 1] << endl;
}
