...
bool isPossibleToPlaceCowsAtDistanceK(int k,int cows,vector<int> &x){

    int dist = 0, i = 1, prev = x[0];
    cows--;
    while(i<x.size()){
        dist = x[i]-prev;
        if(dist>=k) {
...
/* just go answer wise that if its possible
   to place cows with min dis of k each */
...
void solve()
{
    ...
    forn(i, n) cin >> x[i];
    sort(all(x));
    int l=1,h=x[n-1]-x[0],ans=1;
    while(l<=h){
        int mid = (l+h)/2;
        if(isPossibleToPlaceCowsAtDistanceK(mid,c,x)){
            ans = mid;
            l = mid+1;
        }
        else 
