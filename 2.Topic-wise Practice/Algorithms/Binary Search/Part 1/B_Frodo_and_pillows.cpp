...
/*
  x-(k-1)     x-1   x  x-1      x-(n-k)
   _   .....   _    _   _  .....  _
         
               <----|<-------->
                sum1    sum2

    x-(k-1).......x-1-> k-terms => sum1 = ((k-1)*x-k*(k-1)/2)
                                    +x
    x-(n-k)......x-1 -> n-k terms => sum2 = (n-k)*x - (n-k)*(n-k+1)/2
    
   197 198 197       2 1 1
    __  __   _ ......... _
*/
bool giveToAll(int pillow, int n, int m, int k)
{
    int one_to_k = pillow < k ? (pillow * (pillow + 1) / 2 + k - pillow) : (k * (pillow + pillow - k + 1) / 2);
    int k_plus_1_to_n = pillow < n - k + 1 ? ((pillow - 1) * pillow / 2 + (n - k) - (pillow - 1)) : ((n - k) * (pillow - 1 + pillow - 1 - (n - k) + 1) / 2);

    return one_to_k + k_plus_1_to_n <= m;
}
