...
int maximumPathSum(vector<vector<int>> &grid, vector<vector<int>> &dp, int row,int col){
    // base case
    ...
    // transition
    dp[row][col] = grid[row][col]+ max(maximumPathSum(grid, dp, row - 1, col), maximumPathSum(grid, dp, row, col-1));
    // save and return
    return dp[row][col];
...
void solve(){
    ...
    int ans = maximumPathSum(grid,dp,n-1,m-1);
    cout<<ans<<endl;
}
/*
Description
