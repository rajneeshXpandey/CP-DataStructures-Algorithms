...
void solve(){
    ...
    cin>>s;
    int n = s.size();
    map<char,vector<int>> cache; // cahr->indexs
    for(int i=0;i<n;i++){
        cache[s[i]].pb(i);
    }
    int l1,l2,r1,r2;
    map<array<char,2>,int> mp; // {l,r} -> count
    for(char l='a';l<='z';l++){
       for(char r='a';r<='z';r++){
           if(l==r){
                mp[{l,r}] = (cache[r].size()+1)*cache[r].size()/2;
           }
           else{
                for(int i=0;i<cache[l].size();i++){
                    l1 = cache[l][i];
                    r1 = upper_bound(all(cache[r]),l1)-cache[r].begin();
                    mp[{l,r}] += cache[r].size()-r1;
                }
           }
       }
    }
