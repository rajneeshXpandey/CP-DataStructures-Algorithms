...
void solve(){
    ...
    // dp[i][j] -> if sum i is possible with choosing either element from every pair of jth elements of arr 
    // dp[i][j] = 1 if dp[i-arr[j][0]][j-1] || dp[i-arr[j][1]][j-1] == 1
    dp[0][0]=1;
    for(int i = 1; i <= m; i++){
        for(int j = 1; j <= n; j++){
            if(i-arr[j-1][0] >= 0) dp[i][j] = (dp[i][j] || dp[i-arr[j-1][0]][j-1]);
            if(i-arr[j-1][1] >= 0) dp[i][j] = (dp[i][j] || dp[i-arr[j-1][1]][j-1]);
        }
    }
    if(dp[m][n]){
        YES;
