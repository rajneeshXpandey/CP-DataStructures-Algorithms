...
void solve()
{
    ...
    forn(i, m)
    {
        ...
        if (cu == 'R' and cv == 'R')
        {
            roots[u]->left = roots[v];
            roots[v]->left = roots[u];
        }
        else if (cu == 'R' and cv == 'B')
        {
            roots[u]->left = roots[v];
            roots[v]->right = roots[u];
        }
        else if (cu == 'B' and cv == 'R')
        {
            roots[u]->right = roots[v];
            roots[v]->left = roots[u];
        }
        else
        {
    ...
    forn(i, n)
    {
            ...
            while (!q.empty())
            {
                ...
                q.pop();
                cnt++;
                if (roots[node]->left && roots[node]->right)
                {
                    incycle++;
                }
                if (roots[node]->left && !vis[(roots[node]->left)->val])
                {
                    q.push((roots[node]->left)->val);
                    vis[(roots[node]->left)->val] = 1;
                }
                if (roots[node]->right && !vis[(roots[node]->right)->val])
                {
                    q.push((roots[node]->right)->val);
                    vis[(roots[node]->right)->val] = 1;
                }
            }
            if(incycle==cnt)
                inCycle++;
