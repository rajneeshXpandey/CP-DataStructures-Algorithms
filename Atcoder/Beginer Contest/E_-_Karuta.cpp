...
public:
    ...
    TrieNode()
    {
        ...
        forn(i,26) childs[i]=NULL;
        isEnd = false;
         memset(strEnding, 0, sizeof(strEnding));
        //strEnding=0;
    }
    ...
    int strendval(char ch)
    {
        return strEnding[ch-'a'];
    }
};
...
public:
    ...
    {
        TrieNode *node = root;
        vector<bool> update(size(word),0); 
        if (!node->containsKey(word[0])){
            node->put(word[0], new TrieNode());
            //node->strend(word[0]);
        }
        else{
            update[0]=1;
    ...
    int strending(string str)
    {
        ...
        {
            if (node->containsKey(ch)){
                //deb(node->strendval(ch));
                if(node->strendval(ch)>0){
                    maxx++;
                }
                else
                    return maxx;
...
void solve(){
    ...
    vector<string> arr(n);
    forn(i, n) cin >> arr[i];
    Trie* prextree = new Trie();
    forn(i,n){
        prextree->insert(arr[i]);
    }
    int ans=0;
    forn(i,n){
       ans = prextree->strending(arr[i]);
       cout<<ans<<endl;
    }
}
