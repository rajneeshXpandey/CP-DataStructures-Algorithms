...
/*
    ...
    { f(a[1],a[2])+f(a[1],a[3])......f(a[1],a[n]) : n-1
        + f(a[2],a[3])+f(a[2],a[4])......f(a[2],a[n]) : n-2
            + f(a[3],a[4])....
            .
            + f(a[n-1],a[n]) } % K : 1
...
void solve(){
    ...
    vector<int> arr(n);
    int sum=0;
    forn(i, n) cin >> arr[i], sum=(sum+(arr[i]*(n-1)));
    int decrease=0;
    sort(all(arr));
    forn(i,n){
        int j = lower_bound(arr.begin()+i+1, arr.end(), (K - arr[i])) - arr.begin();
        //deb(j);
        decrease+=(n-j);
    }
    //deb2(sum,decrease);
    int ans = (sum-K*(decrease));
    cout<<ans<<endl;
}
