...
int popcorn(vector<string> &arr, vector<int> visit, int idx, vector<int> st){
    if (idx == arr.size()){
        for(auto i: visit){
            if(i==0)
                return inf;
    ...
    //not-took
    int ans = popcorn(arr,visit,idx+1,st);
    // took
    forn(i,arr[idx].size()){
        if(arr[idx][i]=='o') visit[i]=1;
    }
    st[idx]=1;
    ans = min(ans,popcorn(arr, visit, idx+1,st));
    return ans;
}
...
void solve(){
    ...
    vector<string> arr(n);
    forn(i, n) cin >> arr[i];
    vector<int> visit(m,0),st(n,0);
    int ans = popcorn(arr,visit,0,st);
    cout<<ans<<endl;
}
