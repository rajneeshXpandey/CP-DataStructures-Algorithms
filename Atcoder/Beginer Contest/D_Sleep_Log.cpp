...
void solve(){
    ...
    loop(i,1,n-1){
        if(i%2){
            ranges.pb({arr[i], arr[i+1]});
        }
    }
    int sz = ranges.size();
    vector<int> prx(sz+1LL,0LL);
    loop(i,1,sz){
        prx[i] = prx[i-1] + (ranges[i-1].ss - ranges[i-1].ff);
    }
    //printDS(prx);
    int q,l,r;
    cin>>q;
    while(q--){
        cin>>l>>r;
        int li = lower_bound(all(ranges), mp(l,0LL)) - ranges.begin();
        if(li==sz) li--;
        int ri = upper_bound(all(ranges), mp(r,0LL)) - ranges.begin();
        ri--;
        if(li==0LL){
        }
        else if(li>=1LL and ranges[li-1].ss>l){
            li--;
        }
        l = max(l,ranges[li].ff);
        r = min(r,ranges[ri].ss);
        int ans=0;
        if(li>ri){
            cout<<ans<<endl;
            continue;
