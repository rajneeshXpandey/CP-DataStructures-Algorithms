...
void solve()
{
    ...
    for(int i=0;i<n;i++){
        ...
        sz = min(sz,(ll)maxheap.size());
        forn(i,sz){
            heapnodes.pb({maxheap.top()});
            maxheap.pop();
        }
        // printDS(nodes);
        for(auto& pp: heapnodes){
            if(nodes.find(pp.second)==nodes.end()){
                maxheap.push({p[pp.second],pp.second});
            }
        }
        // deb(maxheap.size());
        if(maxheap.empty()){
