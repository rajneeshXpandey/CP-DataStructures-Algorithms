...
void solve(){
    ...
    forn(i, n) cin >> ab[i][0] >> ab[i][1]; // a , b
    // base
    /*
      !! state
          dp[i][k] -> can we make sum k with the remaining i coins and taking j coins of ith type
    */
    // transition
    //dp[i][k] = take 0,1,2----j time the ith element
    // result 
    //dp[n][x]
    ...
    loop(i,1,n){
        loop(curr_sum,0,x){
            loop(j,0,ab[i-1][1]){
                    if((curr_sum-(j*ab[i-1][0]))>=0){
                        dp[i][curr_sum] = (dp[i][curr_sum] || dp[i-1][curr_sum-(j*ab[i-1][0])]);
                    }
            }
        }
    }
