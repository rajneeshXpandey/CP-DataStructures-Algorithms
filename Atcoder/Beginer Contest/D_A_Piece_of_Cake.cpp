...
void solve(){
    ...
    vector<int> a(an);
    a[0]=0;
    loop(i,1,an-2) cin>>a[i];
    a[an-1]=w;
    cin>>bn;
    bn+=2;
    vector<int> b(bn);
    b[0]=0;
    loop(i,1,bn-2) cin>>b[i];
    b[bn-1]=h;
    int minstr=inf,maxstr=-inf;
    map<string,int> xy; 
    forn(i,n){
        int x = lower_bound(all(a),p[i][0])-a.begin();
        int y = lower_bound(all(b),p[i][1])-b.begin();
        x--,y--;
        string k = "";
        k += to_string(x);
        k+="*";
    ...
    int cell=xy.size();
    for(auto p: xy){
        maxstr=max(maxstr,p.ss);
        minstr=min(minstr,p.ss);
    }
    if(cell!=(an-1)*(bn-1)) minstr=0;
