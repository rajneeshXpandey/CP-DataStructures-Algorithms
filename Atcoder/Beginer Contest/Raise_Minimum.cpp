...
#define inf 5000000000000000000 //2e18
#define pi  3.141592653589793238
double eps = 1e-12;

// ********************************* start ********************************** //

bool canMake(vector<int>& arr, int x, int k){
    int operation=0;
    forn(i,arr.size()){
        if(arr[i]<x){
            operation += cdiv(x-arr[i],i+1ll);
            if(operation>k) return false;
        }
    }
    if(operation>k) return false;
    return true;
