...
public:
    ...
    bool containsKey(char ch) { return childs[ch - 'a'] != nullptr; }

    void put(char ch, TrieNode *node) { childs[ch - 'a'] = node; }

    TrieNode *get(char ch) { return childs[ch - 'a']; }

    bool isXends() { return xend; }

    void setXend() { xend = true; }

    void addPendingY(int idx) { pendingY.insert(idx); }

    void collectPendingY()
    {
        for (int idx : pendingY) Z.insert(idx); // Use global Z set
        pendingY.clear();
    }
};
...
public:
    ...
    void insert(string word, int query, int index)
    {
        TrieNode *node = root;
