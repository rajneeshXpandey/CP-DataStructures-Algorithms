...
bool containsCycle(vector<vector<int>> &adj, vector<bool> &visited, int root, int parent)
{
    ...
    for (auto child : adj[root])
    {
        if (!visited[child])
        {
            if (containsCycle(adj, visited, child, root) == true)
                return true;
        }
...
void CountConnectedComponents(vector<vector<int>> &adj, vector<bool> &visited, int root)
{
    visited[root]=1;
    for(auto nb:adj[root]){
        if(visited[nb]!=1)
            CountConnectedComponents(adj,visited,nb);
    }
}
void solve(){
    int n,m,u,v;
    ...
    for (int root = 0; root < n; root++)
    {
        if (!visited[root])
        {
            ...
            CountConnectedComponents(adj, visited, root); // simple dfs on graph
        }
    }
    ...
    visited.clear();
    visited.assign(n,0);
    if(containsCycle(adj, visited, 0, -1)){
        no;
        return;
    }
    yes;
