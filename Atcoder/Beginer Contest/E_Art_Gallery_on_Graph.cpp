...
//Data structures
#define max_heap priority_queue<pii> //maxpq
...
void solve(){
    ...
    vector<array<int, 2>> ph(k);
    forn(i, m){
        cin >> u >> v;
        adj[u].pb(v);
        adj[v].pb(u);
    }
    forn(i, k) cin >> ph[i][0] >> ph[i][1];
    max_heap q;
    vector<int> vis(n + 1, 0);
    vector<int> stamina(n + 1, 0);
    forn(i, k)
    {
        stamina[ph[i][0]] = max(stamina[ph[i][0]], ph[i][1]);
    }
    forn(i, k)
    {
        q.push({stamina[ph[i][0]], ph[i][0]}),
        vis[ph[i][0]] = 1;
    }
    while (!q.empty())
    {
        ...
        if (stamina[t.ss] <= 0)
            continue;
        // if(stamina[t.ss]>t.ff) continue;
        for (auto nb : adj[t.ss])
        {
            if (!vis[nb])
                vis[nb] = 1;
            if (stamina[nb] < stamina[t.ss]-1)
            {
                stamina[nb] = stamina[t.ss]-1;
                q.push({stamina[nb], nb});
            }
        }
    }
    vector<int> guarded;
