...
// ********************************* Here we go!! ********************************** //
int N = 531442; // maximum nums possible
set<string> beautifulNumbers;

void makenum(int i,string &num, int n){
    if(sz(beautifulNumbers)>=n) return;
    if(i>=9){
        beautifulNumbers.insert(num);
        return;
    }
    if(i==0){
        for(int j=1;j<=9;j++){
...
void solve(){
    ...
    cin >> n;
    precompute(n);
    //vector<string> beautifulNumber(all(beautifulNumbers));
    //deb(sz(beautifulNumbers));
    cout<<*(--beautifulNumbers.end())<<endl;
}
