...
int knapsack(vector<int> &w, vector<int> &v, int level, int weight_taken, int n, int W)
{
    // base
    if(level==n){
        return 0;
    ...
    // pruning
    // check cache
    if(dp[level][weight_taken]!=-1) return dp[level][weight_taken];
    // transition
    int max_without_take = knapsack(w,v,level+1,weight_taken,n,W);
    int max_with_take = -inf;
    if(weight_taken + w[level] <= W)
        max_with_take = v[level] +  knapsack(w, v, level + 1, weight_taken + w[level],n,W);
    // save and return
    return dp[level][weight_taken] = max(max_without_take, max_with_take);
}
...
void solve(){
    ...
    int max_sum = knapsack(w,v,0,0,n,W);
    cout<<max_sum<<endl;
}
/*
Description
