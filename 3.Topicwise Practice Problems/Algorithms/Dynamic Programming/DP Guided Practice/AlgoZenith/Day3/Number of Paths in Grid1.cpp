...
int numberOfPaths(vector<vector<int>> &grid, vector<vector<int>> &dp, int row, int col)
{
    // base case
    if (row < 0 or col < 0)
        return 0;
    ...
    if (dp[row][col] != -1)
        return dp[row][col];
    // transition
    dp[row][col] = (numberOfPaths(grid, dp, row - 1, col) 
                        + numberOfPaths(grid, dp, row, col - 1))%mod;
    // save and return
...
void solve(){
    ...
    int ans = numberOfPaths(grid, dp, n - 1, m - 1)%mod;
    cout << ans << endl;
}
/*
Description
