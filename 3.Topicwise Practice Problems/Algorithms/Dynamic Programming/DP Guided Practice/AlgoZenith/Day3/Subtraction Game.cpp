...
// ********************************* Code Begins ********************************** //
//int dp[200001];
bool gameOn(vector<int> &dp,int x){
    ...
    forn(i, log2(x) + 1)
    {
        current_win &= gameOn(dp,(x - (1LL << i)));
    }
    return dp[x] = !(current_win);
}
void solve(){
    int n;
    cin >> n;
    ...
    if(n%3==0){  //observation from dp
        cout<<"Abhishek"<<endl;
    }
    else cout<<"Vivek"<<endl;
    /*
      all moves -> result win  =>  loss
      if any move loss => win
...
x chips on the table. On each turn, a player can pick up
2^m chips such that 
m≥0 and x≥2^m
...
The next T lines contain an integer
x, - the number of chips on the table.
