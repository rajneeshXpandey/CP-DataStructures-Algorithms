...
void solve()
{
    ...
    for(auto city : a){
        int nearest_tower = lower_bound(all(b),city)-b.begin();
        if (city == b[nearest_tower])
            nearest_tower++;
        near = min({(nearest_tower >= 1) ? abs(city - b[nearest_tower - 1]) : inf, (nearest_tower < m) ? abs(city - b[nearest_tower]) : inf, (nearest_tower>=2)?abs(city - b[nearest_tower - 2]):inf});
        r = max(r,near);
        //cout<<r<<' ';
    }
