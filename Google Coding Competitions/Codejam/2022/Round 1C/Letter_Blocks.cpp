...
bool good(string str){
    ...
    for (int j = 1; j < str.size(); j++)
    {
        if (!visi[str[j] - 'A'])
            visi[str[j] - 'A'] = 1;
        else if (visi[str[j] - 'A'] and str[j - 1] != str[j])
        {
...
void solve(){
    ...
    map<char, vector<string>> mp,newmp;
    ...
    int sz = 0;
    for(auto p : mp){
        vector<string> na,temp;
        vector<string> nstr(26,"");
        for(auto word : p.ss){
            set<char> s;
            for (auto w : word)
            {
    ...
    //}
    forn(i,26){
            map<char, vector<string>> st = newmp;
            char start = 'A'+i;
            if (st[start].size()==0){
               continue;
            }
            int cnt=0;
            string ans = "";
            for (auto stg : st[start])
                ans += stg;
            cnt += st[start].size();    
            st[start].clear();
            char prev = ans[(ans.size() - 1)];
            while(cnt<sz){
                while (st[prev].size() > 0)
                {
                    for (auto stg : st[prev])
                        ans += stg;
                    cnt+=st[prev].size();
                    st[prev].clear();
                    prev = ans[ans.size() - 1];
