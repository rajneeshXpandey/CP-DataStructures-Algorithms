...
void generateStringWithRPS(int n, string s, vector<string> &cache){
    if(n == 0){
        cache.push_back(s);
        return;
    }
    if(s.back() == 'R'){
        generateStringWithRPS(n-1, s+"P", cache);
        generateStringWithRPS(n-1, s+"S", cache);

    }
    else if(s.back() == 'P'){
        generateStringWithRPS(n-1, s+"R", cache);
        generateStringWithRPS(n-1, s+"S", cache);
    }
    else{
        generateStringWithRPS(n-1, s+"R", cache);
...
void solve(){
    ...
    int n = s.size();
    vector<string> cache;
    generateStringWithRPS(n-1, "R", cache);
    generateStringWithRPS(n-1, "P", cache);
    generateStringWithRPS(n-1, "S", cache);
    int ans=inf, tans;
    for(auto &str : cache){
        tans = 0;
        if(str[0] == str[n-1]) continue;
        forn(i, n){
            tans += (str[i] != s[i]);
        }
        //deb3(str, s,tans);
