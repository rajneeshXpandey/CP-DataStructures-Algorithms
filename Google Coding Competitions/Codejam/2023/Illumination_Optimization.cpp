...
void solve(){
    ...
    int right = -1,idx=0;
    int cnt = 0;
    if(max(0LL,arr[idx]-r)>0){
        cout<<"IMPOSSIBLE"<<endl;
        return;
    }
    while(idx<n and max(0LL,arr[idx]-r)==0){
        idx++;
    }
    if(idx==n) {
        right = min(m, arr[idx-1]+r);
        cnt++;
        idx--;
    }
    else{
        if(max(0LL,arr[idx]-r)==0){
            right = min(m, arr[idx]+r);
            cnt++;
        }
        else{
            right = min(m, arr[idx-1]+r);
    ...
    loop(i,idx+1,n-1){
        if(right==m) break;
        if(max(0LL,arr[i]-r)>right){
            cout<<"IMPOSSIBLE"<<endl;
            return;
        }
        else if(max(0LL,arr[i]-r)==right){
            right = min(m, arr[i]+r);
            cnt++;
        }
        else if(max(0LL,arr[i]-r)<right){
            while(i<n and max(0LL,arr[i]-r)<right){
                i++;
            }
            if(i==n){
                if(max(0LL,arr[i-1]-r)<=right){
                    right = min(m, arr[i-1]+r);
                    cnt++;
                }
                else{
                    cout<<"IMPOSSIBLE"<<endl;
