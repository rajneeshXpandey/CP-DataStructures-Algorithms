...
// ********************************* Code Begins ********************************** //
int dfs(vector<vector<int>> &adj, vector<bool> &visited, vector<int> &cir, int root)
{
    visited[root]=1;
    int cnt=1;
...
void solve(){
    ...
    vector<int> circulated(n,0);
    vector<bool> visited(n,0);
    int ans=1;
    for (int i = 0; i < n; i++)
        if (!visited[i]){
            int cc = dfs(adj,visited,circulated,i);
            ans = max(ans,cc);
            //deb(cc);
        }
    //printDS(circulated);    
