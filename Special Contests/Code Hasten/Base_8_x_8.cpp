...
// ********************************* Here we go!! ********************************** //
inline ll power(ll a, ll n, int modd){a %= modd; int res = 1; while(n){if (n & 1) res = (res * a) % modd; a = (a * a) % modd;n >>= 1;} return res;}
...
void solve(){
    ...
    int ans = 0;
    forn(i,n){
        ans += (power(64,n-1-i,9) * value[s[i]]);
        ans %= 9;
    }
    cout << ans << endl;
...
signed main(){
    ...
    for(char i = 'A'; i <= 'Z'; i++) value[i] = i - 'A';
    for(char i = 'a'; i <= 'z'; i++) value[i] = i - 'a' + 26;
    for(char i = '0'; i <= '9'; i++) value[i] = i - '0' + 52;
    value['+'] = 62;
    value['/'] = 63;
