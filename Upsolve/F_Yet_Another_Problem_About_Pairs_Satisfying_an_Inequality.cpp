...
void solve(){
    ...
    forn(i,n){
        if(((i+1)-a[i])>0)
            aminusi.pb({((i+1)-a[i]),(i+1)});
    }
    // sort(all(aminusi));
    int ans = 0;
    for(int i=0;i<sz(aminusi);i++){
        cout<<"("<<aminusi[i].first<<","<<aminusi[i].second<<")"<<" ";
    }
    cout<<endl;
    forn(i,sz(aminusi)){
        int x = aminusi[i].ff;
        int ind = aminusi[i].ss;
        loop(k,0,sz(aminusi)-1){
            if(k!=i and x<ind and ind<(aminusi[k].ss-aminusi[k].ff) and (aminusi[k].ss-aminusi[k].ff)<aminusi[k].ss){
                ans++;
            }
        }
    }
