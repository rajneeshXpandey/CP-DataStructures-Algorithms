...
int calc1(int n,int k){
    ...
    if(sz>=n){
        for (int i = 0; i < n-1; i++)
        {
            tans += setbits(resource[i]), k -= resource[i];
        }
        tans += setbits(k);
    }
    else {
...
void solve()
{
    ...
    vector<int> resource;
    int ans=0;
    for (int i = 0; i <= setbits(k) and (k-i>=0) and (n-i>0); i++)
    {
        ans = max(ans,calc1(n-i,k-i)+i);
        ans = max(ans,calc1(n-i,k-i)+i);
    }
    if(k-(n-1)>=0)
      ans=max(ans,(n-1)+setbits(k-(n-1)));
    if(n>=k) ans = max(k,ans);
    cout<<ans<<endl;
}
