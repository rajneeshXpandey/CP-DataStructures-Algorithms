...
int minOp(string a, string b,int n){
    int op=0;
    forn(i,n){
        if(a[i]!=b[i]){
            op++;
...
void solve(){
    ...
    int ans=0;
     for(int i=0,j=n-1;i<n and j>=0;i++,j--){
            string col="",col1="";
            string row="",row1="";
            row += grid[i].substr(i,n-i);
            row1 += grid[i].substr(0,n-i);
            loop(k,i,n-1) col += grid[k][j];
            loop(k,0,n-i-1) col1 += grid[k][j];
            int a1 = minOp(row,col,n);
            int a2 = minOp(row1,col1,n);
