...
void solve(){
    ...
    forn(i,n){
        int low_idx = lower_bound(all(a),i)-a.begin();
        int up_idx = upper_bound(all(a),i)-a.begin();
        int maxx=0,minn=0;
        int same=up_idx-low_idx;
        int left = low_idx;
        int right = n-up_idx;
        minn = right;
        maxx = (left+right)*n - (prex[n]-prex[up_idx] - right) - prex[low_idx]; 
        cout<<minn<<' '<<maxx<<endl;
    }
}
signed main(){
