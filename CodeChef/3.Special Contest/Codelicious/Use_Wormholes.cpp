...
// ********************************* start ********************************** //
int getminAns(int i, int j, int n, vector<vector<int>> &arr, vector<vector<int>> &dp)
{
    if(i==n) return 0;
        // choose two nearest points from arr[k] to start
    if(dp[i][j]!=-1){
        return dp[i][j];
    }

    int time=inf;
    int close = lower_bound(all(arr[i]), arr[i-1][j]) - arr[i].begin();
    for(int l=max(0LL,close-2);l<=min((ll)arr[i].size()-1,close+2);l++){
        time = min(time,abs(arr[i-1][j]-arr[i][l])+getminAns(i+1,l,n,arr,dp));
    }
    return dp[i][j] = time;
}
void solve(){
    ...
    vector<vector<int>> dp(n,vector<int>(maxk+1,-1));
    int time=inf;
    forn(j,arr[0].size()){
        // choose arr[0][i] as starting point
        time = min(time, getminAns(1,j,n,arr,dp));
    }
    //if(time==inf) time=0;
    cout<<time<<endl;
}
