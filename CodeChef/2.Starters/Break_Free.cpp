...
inline void binary(ll n) { string binaryMask = bitset<64>(n).to_string(); cout<<binaryMask<<endl;}
...
void solve(){
    ...
    forn(i, n) cin >> arr[i];
    int x = 0;
    forn(i,n) if(arr[i]%2==0) x++;

    int ans;
    if (n == x)
        ans = (power(2,x,mod)-1)%mod;
    else
        ans = power(2,x,mod) % mod;
    cout << ans << endl;
}
