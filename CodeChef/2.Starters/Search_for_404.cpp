...
void solve(){
    ...
    cin>>s;
    /*
      case works and counting:
       ans += cnt(404) * 2^(stars)
           += (cnt(4*4) * 2^(stars-1)) + (cnt(*04) * 2^(stars-1)) + (cnt(40*) * 2^(stars-1))
           += (cnt(**4) * 2^(stars-2) + cnt(4**) * 2^(stars-2) + cnt(*0*) * 2^(stars-2))
           += cnt(***) * 2^(stars-3)

        cnt :: cnt40 => if(s[i]==4) cnt4++, if(s[i]==0) cnt40+=cnt4;
            :: cnt404 => if(s[i]==4) cnt4++, if(s[i]==0) cnt40+=cnt4, if(s[i]==4) cnt404+=cnt40;
    */
    int cnt_star = 0, cnt4 = 0, cnt40 = 0, cnt404 = 0;

    for (char ch : s)
    {
        if (ch == '*')
        {
            cnt404 = (cnt404 * 2 + cnt40) % mod;
            cnt40 = (cnt40 * 2 + cnt4) % mod;
            cnt4 = (cnt4 * 2 + power(2,cnt_star,mod)) % mod;
            cnt_star += 1;
        }
        else if (ch == '4')
        {
            cnt404 = (cnt404 + cnt40) % mod;
            cnt4 = (cnt4 + power(2,cnt_star,mod)) % mod;
        }
        else if (ch == '0')
        {
            cnt40 = (cnt40 + cnt4) % mod;
        }
    }
    cout<<cnt404<<endl;
}
