...
// ********************************* start ********************************** //
// generate all possible binary strings made up of b where set bit in b can be set or unset
void getallbs(int &a,int &sz,int b,int pos,int &l,int &r,set<int> &x){
    int ans = 0;
    if(pos == sz){
        if(b >= l and b<=r and a<=b){
            x.insert(a+b);
        }
        return;
    }
    if((a & (1LL << pos))==0){
        getallbs(a,sz,b,pos+1,l,r,x);
        b = (b | (1LL << (pos)));
        getallbs(a,sz,b,pos+1,l,r,x);
    }
    else getallbs(a,sz,b,pos+1,l,r,x);
}
void solve(){
    int l,r;
    cin >> l >> r;
    if(l==0 and setbits(r+1LL)==1){
        cout << r+1LL << endl;
        return;
    }
    if(l==0){
        cout << (1LL<<(int)log2(r)+1) << endl;
        return;
    }
    int sz = log2(r) + 1LL;
    set<int> x;
    loop(a,l,r){
        getallbs(a,sz,0LL,0LL,l,r,x);
    }
    cout << x.size() << endl;
}
