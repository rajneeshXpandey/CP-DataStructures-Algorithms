...
template<typename T> inline void printPDS(T dataStructure){for(auto val : dataStructure) deb2(val.ff, val.ss);}
...
void solve(){
    ...
    unordered_map<int,vector<array<int,2>>> mp; // sum, pair of index
    forn(i, n){
        loop(j, i+1, n-1){
    ...
    int ans = inf;
    for(auto p : mp){
        sort(all(p.ss),[](array<int,2> &a, array<int,2> &b){
            return a[1]<b[1];
        });
        int previdx = -1;
        int take=0;
