...
void solve()
{
    ...
    //loop(_k, 1, k)
    //{
    //    ans = max({ans, _k * (2 * n - 1) - (2*_k*_k)});
    //    deb2(_k, ans);
    //}
    /*
    quard = k*(2*n+1)-2(k*k)
    root of poly => 0 and (2*n+1)/2
        max at k = (2*n+1)/4;
        check max at given k(if(k>=(2*n+1)/4) else at k)
    */

     ans = max({ans,k*(2*n-1)-(2*k*k)});
     if(k>=(2*n-1)/4){
         ans = max(ans,(2*n-1) * (2*n-1)/8);
         if(k>=(2*n+3)/4){
             ans = max(ans, (((2 * n + 3) * (2 * n - 1)) / 4) - ((2 * n + 3) * (2 * n + 3) / 8));
         }
    }
    cout << ans << endl;
}
