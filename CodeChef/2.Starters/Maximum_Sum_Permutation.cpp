...
void solve(){
    ...
    vector<int> newarr(n);
    sort(all(arr));
    map<int, vector<int>> prior;
    loop(i,1,n) prior[prefix[i]].pb(i-1);
    int arrptr = 0;
    for(auto p : prior){
        for(auto idx : p.ss){
            newarr[idx] = arr[arrptr++];
        }
    }
    int x = 0;
    int newprefix[n+1];
    ...
    loop(i,1,n){
        newprefix[i] = newprefix[i-1] + newarr[i-1];
    }
    forn(i,q){
        x += (newprefix[queries[i][1]] - newprefix[queries[i][0]-1]);
    }
    cout << x << endl;
    printDS(newarr);
}
