...
void solve(){
    ...
    int k = setbits(n);
    /*
        distribute k bits to a,b,c such that a+b+c = n
        3^k ways to distribute k bits to a,b,c
        but (3C2 * 2^k) where a/b/c gets 0 setbits
        so total ways = 3^k - (3C2 * 2^k) 
        now add ways if two of them get 0 setbits = 3
    */
     int ans = (power(3,k,mod)+3)%mod;
     ans = (ans - (3*power(2,k,mod))%mod + mod)%mod;
     cout << ans << endl;
}
