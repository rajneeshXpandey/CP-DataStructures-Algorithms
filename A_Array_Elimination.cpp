...
void solve(){
    ...
    loop(k,1,n){
        ...
        if(!notpossible) cout<<k<<' ';
    }
    cout<<endl;
}
/**
    for ith position :- 
        all the k element should have the bit = 1 so that after subtraction all can be 0

    => now all of n elements to be 0 each set of chosen k element,
         should be multiple of the total numbers of 1 that needs to be made 0 in batch size of k
